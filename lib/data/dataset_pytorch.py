...
from lib.data import get_custom_aug

from lib.data import HeatmapProcessor
import cv2
import torch
...
#                  outputHeatmap=False,
#                  heatmapRes=None,
#                  heatmapSigmaRange=(3.0, 1.5, -0.05),
#                  leftRightJointPairIndexes=None,
#                  probFlipH=0.0,
#                  probMaskRandom=0.0,
#                  probGaussianNoise=0.0,
...
#         sigmaStart, sigmaEnd, sigmaStep = heatmapSigmaRange
#         self.heatmapSigmaSchedule = np.arange(sigmaStart, sigmaEnd, sigmaStep)
#         self.heatmapSigmaIdx = 0
#         self.heatmapSigma = self.heatmapSigmaSchedule[0]
...
#         imgFile = self.imgPaths[idx]
#         keyPts = self.keyPts[imgFile].copy()
...
#         # Normalize image and key point according to the pretrained weight config.
#         if self.normalizeImg:
...
#             img[:,:,1]=((img[:,:,1]-mean1)/std1)
#             img[:,:,2]=((img[:,:,2]-mean2)/std2)
#         # resizing the image according to the pretrained weight config
#         if self.resize:
#             dim=self.config['input_size'][1:]
